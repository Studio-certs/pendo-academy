/*
  ...
  1. Changes
    - Improve the handle_application_approval function
    - Add proper error handling and transaction management
    - Ensure tokens are deducted correctly
    - Create transaction record for token deduction
    - Properly create course enrollment with application_id reference
...
BEGIN
  ...
  IF NEW.status = 'approved' AND OLD.status = 'pending' THEN
    ...
    IF wallet_id IS NULL THEN
      RAISE EXCEPTION 'User wallet not found for user ID %', NEW.user_id;
    END IF;
